...
def upload_product_image():
    """Endpoint pro nahrání obrázku produktu"""
    current_app.logger.info('Přístup k nahrání obrázku produktu')
    
    # Kontrola existence souboru
    if 'image' not in request.files:
        current_app.logger.error("Chybí soubor s obrázkem")
        return jsonify({'error': 'Chybí soubor s obrázkem'}), 400
